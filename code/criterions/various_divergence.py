...
class VariousDivergence(CrossEntropyLoss):
    ...
    def forward(
        ...
        ce_loss = self.compute_cross_entropy_loss(
            logits, batch["label_batch"]["label"], reduction="sum"
        )[0] / batch["label_batch"]["loss_denom"]
        log["nll_loss"] = ce_loss

        if "op_input_batch" in batch:
            op_outputs = model(**batch["op_input_batch"], output_hidden_states=True)
            op_logits = op_outputs.logits
            with torch.no_grad():
                op_teacher_outputs = teacher_model(
                    **batch["op_teacher_input_batch"], 
                    output_hidden_states=True
                )
            op_teacher_logits = op_teacher_outputs.logits
            # prune logits with different sizes (only for qwen)
            if op_logits.size(-1) != op_teacher_logits.size(-1):
                min_logit_size = min(op_logits.size(-1), op_teacher_logits.size(-1))
                op_logits = op_logits[:, :, :min_logit_size]
                op_teacher_logits = op_teacher_logits[:, :, :min_logit_size]
            
            kd_loss = self.dist_func(op_logits, op_teacher_logits, batch["op_label_batch"]["label"])
            kd_loss = kd_loss / batch["op_label_batch"]["loss_denom"]
        else:
            with torch.no_grad():
            ...
            if logits.size(-1) != teacher_logits.size(-1):
                min_logit_size = min(logits.size(-1), teacher_logits.size(-1))
                logits = logits[:, :, :min_logit_size]
                teacher_logits = teacher_logits[:, :, :min_logit_size]
            
            

            kd_loss = self.dist_func(logits, teacher_logits, batch["label_batch"]["label"])        
            kd_loss = kd_loss / batch["label_batch"]["loss_denom"]
